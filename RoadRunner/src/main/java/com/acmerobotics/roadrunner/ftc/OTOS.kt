...
class OtosImu(private val otos: SparkFunOTOS) : IMU {
    ...
    override fun getRobotAngularVelocity(angleUnit: AngleUnit?): AngularVelocity {
        return AngularVelocity(AngleUnit.RADIANS, 0.0f, otos.velocity.h.toFloat(), 0.0f, System.nanoTime()).toAngleUnit(
            angleUnit
        )
    }
...
/**
 * Originally conversions were done here, but now they should be done with extension functions
 * Now this wraps that for legacy support
 */
fun OTOSPoseToRRPose(otosPose: SparkFunOTOS.Pose2D): Pose2d {
    return otosPose.toRRPose()
}

fun RRPoseToOTOSPose(rrPose: Pose2d): SparkFunOTOS.Pose2D {
    return rrPose.toOTOSPose()
}